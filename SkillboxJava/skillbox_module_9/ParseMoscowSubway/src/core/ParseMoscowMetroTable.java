...
public class ParseMoscowMetroTable {
    ...
    public static ParseResult parse(String url) throws IOException {
        ParseResult tableToObject = new ParseResult();
        Connection wikiMoscowMap = Jsoup.connect(url);
        ...
        tables.forEach(table -> table.select(TABLE_ROW).forEach(row -> {
            ...
            if (!tds.isEmpty()) {
                ...
                tableToObject.rows.add(stationInfo);

            }
        }));
        // sort to Station & Line objects
        tableToObject.stationsToLine();
        ParseResult finalResult = tableToObject.parseConnectionsFix();
        return finalResult;
    }
